...
$(document).ready(function(){
    ...
    /*处理TextArea模拟器*/
    $('.textarea-wrap-box').on('keyup mouseout blur',function(){
        $taContent=this.innerText;
        $(this).closest('.input-group').children('#textarea-content').each(function(i){
            this.value=$taContent;
        })
    })
    ...
    /*媒体上传文件*/
    $('.upload-file-select').on('click',triggerFileDialog);
    $('.picture-shower').on('click',triggerFileDialog);
    $('.portrait-shower').on('click',triggerFileDialog);
    $('.video-shower').on('click',triggerFileDialog);

    function triggerFileDialog(e){
        var hiddenFile=$(this).parent().children('input[type="file"]');
        hiddenFile.click();
    };
